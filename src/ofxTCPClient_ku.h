...
//------------------------------------------------------------
//ofxTCPClient from ofxNetwork addon modification, just for this:
...
class ofxTCPClient_ku {
	...
	public:
		...
		//the receiver see: STR_END_MSG (ofxTCPClient_ku.h)
		...
		ofxTCPManager	&TCPClientRef() { return TCPClient; }	//Added by Kuflex

private:
		// private copy so this can't be copied to avoid problems with destruction
		ofxTCPClient_ku(const ofxTCPManager & mom){};
		ofxTCPClient_ku & operator=(const ofxTCPClient_ku & mom){return *this;}
